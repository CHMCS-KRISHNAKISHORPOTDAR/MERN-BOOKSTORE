...
const BookCard = ({ book, setBooks }) => {
  ...
  return (
    <>
      ...
      <Link to={`/book/${book._id}`} className={`hover:scale-110 relative block rounded-xl bg-base-100 p-4 border transition-all duration-200 ${isActive ? "border-primary shadow-lg" : "border-base-300"}
    hover:border-primary hover:shadow-xl`}>
      ...
      {showModal && (<dialog className="modal modal-open">
        <div className="modal-box">
          <h3 className="font-bold tet-lg text-error flex items-center gap-2">
            <Trash2 className="size-5" />Delete Book </h3>
          <p className="py-4 text-base-content/70">
            Are you sure you want to delete
            <span className="font-semibold text-base-content">{""}"{book.title}" </span>? <br />This action cannot be undone.</p>
